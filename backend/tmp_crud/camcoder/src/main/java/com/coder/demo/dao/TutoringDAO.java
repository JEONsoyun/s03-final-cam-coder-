...
public interface TutoringDAO extends JpaRepository<Tutoring, Long>{
	...
	@Query("select r from Tutoring r INNER JOIN r.tstudent s where s.userCode = ?1 order by r.status asc, r.tutoringCode asc")
	List<Tutoring> findByStudentCode(Long code);
	
	@Query("select r from Tutoring r INNER JOIN r.tteacher t where t.teacherCode = ?1 order by r.status asc, r.tutoringCode asc")
