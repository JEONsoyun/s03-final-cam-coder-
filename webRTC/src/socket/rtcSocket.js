...
const debug = require("debug")("rtcSocket");

function init(namespace, io) {
  let rooms = {};
  let screenInfo = {};
  let roomId = null;
  let socketIds = {};
   ...
   * 소켓 연결
   */
  const nsp = io.of(namespace).on("connection", (socket) => {
    // 룸접속
    socket.on("enter", (roomName, userId) => {
      ...
      // 유저 정보 추가
      nsp.in(roomId).emit("join", roomId, thisRoom);
