    ...
    <div class="teacher-detail-page__container">
      <div class="teacher-detail-page">
          ...
          <div class="d-flex teacher-detail-page__right-content">
            ...
            >
              <div class="d-flex flex-column flex-grow-1 justify-center align-center">
                <div>
                  <span style="color: #fb8805;font-weight:800">{{ SAMPLE_DATA.user.userName }}</span>
                  <span>선생님에게</span>
                </div>
        ...
        <div class="d-flex teacher-detail-page__box">
          <div><img :src="SAMPLE_DATA.profile" /></div>
          <div>
            <div>{{ SAMPLE_DATA.user.userName }}</div>
            <div v-html="SAMPLE_DATA.intro"></div>
            <div>{{ SAMPLE_DATA.expertise }}</div>
            <div>{{ SAMPLE_DATA.avaliableTime }}</div>
            <div>like {{ SAMPLE_DATA.likeCnt }}</div>
            <div>student {{ SAMPLE_DATA.studentCnt }}</div>
          </div>
        </div>
      </div>
    </div>
